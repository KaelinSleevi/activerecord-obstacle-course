...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    users = User.order(:name).joins(:orders).where("orders.user_id = users.id").pluck(:name).uniq
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    items_for_user_3_third_order = Order.joins(:items).where(user_id: @user_3.id).distinct.third.items.pluck(:name)
    # ------------------------------------------------------------
